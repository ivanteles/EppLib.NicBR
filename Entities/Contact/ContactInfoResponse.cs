...
namespace EppLib.Entities.Contact
{
    public class ContactInfoResponse : EppResponse
    {
        ...
        protected override void ProcessExtensionNode(XmlDocument doc, XmlNamespaceManager namespaces)
        {
            ...
            namespaces.AddNamespace("brorg", "urn:ietf:params:xml:ns:brorg-1.0");

            var infData = doc.SelectSingleNode("/ns:epp/ns:response/ns:extension/brorg:infData", namespaces);
            if (infData != null)
            {
                Contact.BrOrg = new BrOrg();
                var pOrganization = infData.SelectSingleNode("brorg:organization", namespaces);
                if (pOrganization != null)
                {
                    Contact.BrOrg.Organization = pOrganization.InnerText;
                }

                var pContact = infData.SelectSingleNode("brorg:contact", namespaces);
                if (pContact != null)
                {
                    Contact.BrOrg.Contact = pContact.InnerText;

                    if (pContact.Attributes["type"] != null)
                        Contact.BrOrg.ContactType = pContact.Attributes["type"].Value;
                }

                var pResponsible = infData.SelectSingleNode("brorg:responsible", namespaces);
                if (pResponsible != null)
                {
                    Contact.BrOrg.Responsible = pResponsible.InnerText;
                }
            }
        }
    }
